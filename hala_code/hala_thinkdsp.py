...
from fractions import gcd # ational numbers
from wave import open as open_wave # Read and write WAV files
...
# a spectrogram, which shows time on the x-axis and frequency on the y-axis.
...
class Wave:
    ...
    def convolve(self, other):
        ...
        if isinstance(other, Wave):
            assert self.framerate == other.framerate #check dtypes,size
            window = other.ys
        else:
            window = other
...
class Signal:
    ...
    def make_wave(self, duration=1, start=0, framerate=11025):
        ...
        ts = start + np.arange(n) / framerate # add first
        ys = self.evaluate(ts) 
        # evaluate 这个函数定义在了继承类里，所以Signal类不是直接给用户使用的。
...
class Sinusoid(Signal):
    ...
    @property # 装饰器， 这样写就可以sig.period 来获得和设置周期
    def period(self):
